...
TAG
        ...
        jLog::log(var_export($categories, true), 'default');
        foreach($categories as  $keycat => $cat){
            $cat = str_replace('-',' ', $cat->tag);

            $xmlstring = Shopping::getProductByKeyword($cat, $_SERVER['HTTP_USER_AGENT'], $_SERVER['REMOTE_ADDR']);

            $sxe = simplexml_load_string($xmlstring, 'SimpleXMLIterator');
            jLog::log(var_export($sxe, true), 'default');
            if($sxe !== false){
                foreach(new RecursiveIteratorIterator($sxe, 1) as $name => $data) {
                    if($data->items->offer !== null){
                        foreach($data->items->offer as $d){
                            //jLog::log(var_export($d,true) ,'default');

                            jLog::log(var_export((string) $d->name,true) ,'default');
                            jLog::log(var_export((string) $d->offerURL,true) ,'default');
                            jLog::log(var_export((string) $d->basePrice,true) ,'default');
                            jLog::log(var_export((string) $d->originalPrice,true) ,'default');
                            jLog::log(var_export((string) $d->shippingCost,true) ,'default');

                            if(isset($d->description)){
                                jLog::log(var_export((string) $d->description,true) ,'default');
                            }

                            if(isset($d->storeNotes)) {
                                jLog::log(var_export((string)$d->storeNotes, true), 'default');
                            }
